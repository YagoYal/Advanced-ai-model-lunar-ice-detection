...
catalogued PSRs correctly detected. On the random 80/20 validation split
(F1 = 0.792, Recall = 0.973, val\_loss = 0.337 at 30 epochs), performance is
lower than an earlier variant that included raw latitude as an input feature
(F1 = 0.997) — deliberately: that variant used latitude as a lookup shortcut
and failed completely on a polar quadrant withheld from training
(Section~\ref{sec:ood}), whereas the reported model generalises to that
regime. The platform
...
\begin{itemize}
  \item \textbf{Dataset:} 18{,}896 examples; 4{,}724 positives (25\%) — sampled
    from a base grid of 14{,}656 PSR-confirmed positive pixels (Table~\ref{tab:labels})
    via the oversampling scheme in \texttt{LunarDataset}
...
\begin{table}[H]
\centering
\caption{CNN training and validation metrics (30 epochs, GPU), after the
  \texttt{lat\_norm} ablation described in Section~\ref{sec:ood} —
  precision 0.668.}
\label{tab:results}
\begin{tabular}{lr}
\toprule
...
Cabeus      & \citet{colaprete2010} & 0.817 & ICE & \checkmark \\
Shackleton  & \citet{gladstone2010} & 0.906 & ICE & \checkmark \\
Haworth     & \citet{spudis2010}    & 0.874 & ICE & \checkmark \\
Nobile      & \citet{hayne2015}     & 0.793 & ICE & \checkmark \\
Amundsen    & \citet{mazarico2011}  & 0.782 & ICE & \checkmark \\
\midrule
\multicolumn{5}{l}{\textit{North pole — confirmed ice}} \\
Hermite     & \citet{paige2010}     & 0.982 & ICE & \checkmark \\
Peary       & \citet{spudis2010}    & 0.855 & ICE & \checkmark \\
Whipple     & \citet{mazarico2011}  & 0.982 & ICE & \checkmark \\
\midrule
...
Table~\ref{tab:benchmark} — scored 11/12, with only Sylvester (a PSR outside the
main benchmark catalogue) misclassified, and attribution was spread across all
five features with \texttt{lat} the single largest contributor (28.2\% for
positives, 31.6\% for negatives) — itself later understood as an artefact,
discussed next.

Figures below reflect the model after the \texttt{lat\_norm} mitigation
(Section~\ref{sec:ood}): the interpretability probe set still scores 11/12,
with Sylvester the sole miss, unchanged from before the mitigation — the
precision trade-off visible in the aggregate metrics (Section~\ref{sec:results})
does not manifest as a new miss on this particular 12-site probe set. The SHAP
GradientExplainer's background distribution required a matching fix: it
originally sampled all five dimensions as $\mathrm{Uniform}(0, 0.5)$, including
\texttt{lat\_norm}. Since \texttt{lat\_norm} is now a constant zero at every
training step, its \texttt{PhysicsEncoder} input weight never receives a
gradient ($\partial \mathcal{L}/\partial w = \partial \mathcal{L}/\partial h
\cdot x$, $x=0$ always) and remains at its random initial value — a background
that varies this dimension probes an untrained weight and reports spurious
attribution for a feature the model cannot see at inference. Before this fix,
\texttt{lat} still showed up as a top contributor (21.7\% positives, 24.8\%
negatives) despite contributing nothing to the actual forward pass — a second,
independent artefact from the same root cause as the OOD failure, but in the
explanation layer rather than the model itself. Zeroing \texttt{lat\_norm} in
the SHAP background to match the true training distribution eliminates it:
\texttt{lat} attribution drops to exactly 0.0\% for both groups, and
\texttt{insolacao} is confirmed as the dominant signal (39.3\% positives,
33.5\% negatives), followed by the subsurface temperature features
(17.2--21.9\% positives, 16.1--33.2\% negatives). Subsurface thermal and
insolation features now account for all attribution, consistent with the model
using physically grounded signal rather than latitude to distinguish PSRs from
non-PSR sites. Calibration on the validation set gives ECE~$\approx 0.087$
(previously $0.0905$ before the mitigation; we observed values from 0.087 to
0.096 across repeated runs, since \texttt{model/run\_interpret.py} does not
seed the validation split or the Monte Carlo Dropout passes) — mild
...
The initial result was a genuine negative finding. With \texttt{lat\_norm} present
as one of the five \texttt{PhysicsEncoder} input features, training without any
south-polar example and testing only on that quadrant (56.6\% positive) yielded
$F_1 = 0.000$ across all 30 epochs — the model detected no ice at all in a
latitude range absent from training. The mirrored north-polar fold was unstable
rather than reliably better: $F_1$ oscillated between $\approx 0.76$ and $0.000$
across epochs rather than converging monotonically. We attributed this to
\texttt{lat\_norm} acting as a near-lookup signal — the model could shortcut to
"this latitude band is polar $\Rightarrow$ ice" instead of learning the
transferable subsurface-thermal and insolation physics that generalise to an
unseen quadrant.

We tested the shortcut hypothesis directly: zeroing \texttt{lat\_norm} at every
feature-construction site (training, inference, benchmark, validation —
column retained at index 1 for input-shape compatibility with the exported ONNX
graph, but always 0) forces the model to rely solely on insolation and the
subsurface thermal profile, both already latitude-dependent through the physics
of Section~\ref{sec:thermal-model} rather than through the raw coordinate. Retrained
under this ablation, the withheld south-polar quadrant improved from
$F_1 = 0.000$ to $F_1 = 0.808$ (recall $= 0.976$, precision $= 0.689$), and the
north-polar fold improved from an unstable $F_1 = 0.731$ to a stable
$F_1 = 0.767$ (recall $= 0.978$, precision $= 0.632$) that no longer collapses
across epochs. (These figures move by $\approx \pm 0.01$ between independent
retrainings, since both weight initialisation and the 80/20 split are
unseeded — we report one representative run rather than an average across
retrainings.) This confirms \texttt{lat\_norm} was the shortcut, not merely a
correlate of one.

This fix was adopted for the production model reported throughout this paper —
the headline metrics in Section~\ref{sec:results} already reflect training
without \texttt{lat\_norm}. The trade-off is a lower random-split $F_1$
(0.997 $\rightarrow$ 0.792, precision 0.668, recall 0.973, val\_loss
0.0109 $\rightarrow$ 0.337) than the earlier lookup-assisted model achieved,
because latitude was informative for the in-distribution split even though it
was also the shortcut that broke out-of-distribution generalisation. The
14-site published-catalogue benchmark (Table~\ref{tab:benchmark}) is unaffected
and remains 14/14 (100\%) after retraining, indicating the drop in the
aggregate random-split metric is concentrated in ambiguous mid-confidence
regions of the grid rather than in physically well-characterised sites. We
consider generalisation to unseen polar regions — the regime lunar missions
actually operate in, since no mission surveys every PSR during training — a
more meaningful correctness criterion than the random-split aggregate, and
report the trade honestly rather than reverting to the higher but
non-generalising number.
